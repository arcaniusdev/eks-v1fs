...
This build scales the scanner on the chart's CPU/memory HPA (TrendAI-supported),
and the simplest integration is a single reused SDK connection (§9). If your
client is a busy, queue-fed service that would otherwise pin all its scans to
one scanner pod, evaluate the **python-KEDA** / **java-KEDA** options instead —
they add queue-depth scanner scaling and a client-side pull/semaphore dispatcher.
...
A minimal end-to-end client is in [`reference/python-default/`](.) (`client_example.py`). This build scales the scanner on the chart's CPU/mem HPA, so a single reused connection is the normal integration; if you later need to fan a high-volume client across many scanner pods without an L7 hop, switch to the **python-KEDA** option (queue-depth scaling + the pull/semaphore dispatcher).
...
The scanner scales on the Helm chart's own autoscaler (CPU and memory targets,
1–10 pods by default) — TrendAI's supported mechanism — and the cluster adds
nodes through the standard Cluster Autoscaler.

| What you'll observe | Why it's normal |
|---|---|
| Scale-up takes **1–3 minutes** under sustained load | the HPA reacts to a metrics window, then node provisioning adds ~1–2 min when new capacity is needed |
| Nothing is lost during a burst | in pipeline mode the SQS queue holds the backlog and drains as capacity arrives |
| Repeat scans of the same file are near-instant | the scanner caches verdicts by file hash; use unique files for honest benchmarks |

Raise the ceilings with `ScannerMaxReplicas`, `ScannerAppMaxReplicas`, and
`NodeGroupMaxSize`. For queue-depth scanner scaling instead of CPU, use the
**python-KEDA** / **java-KEDA** options (`ScannerScalingMode=keda`).
