...
- **`ScannerEndpointMode=alb`** — chart `scanner.ingress` with `className=alb`, GRPC backend-protocol-version, internal scheme, ACM cert. L7 ALB terminates HTTP/2 and load-balances **per gRPC call**, so it defeats the L4 connection-pinning hot-spot that ClusterIP/NLB have with long-lived gRPC connections. Requires `ScannerDomain` plus a cert; the user creates a DNS CNAME to the ALB. TLS: `amaas.grpc.init("<domain>:443", api_key, True)`. NOTE: Trend's *documented* File Security EKS LB is the NLB (L4, for ICAP) — ALB is a valid chart-native gRPC path but is not what Trend's FS docs foreground
- **Self-signed ALB cert (`SelfSignedScannerCert=true`)** — removes the need to bring a publicly-signed cert. When alb mode is set with no `ACMCertificateArn`, `bootstrap.sh` generates a self-signed cert with `SAN=ScannerDomain`, imports it to ACM for the ALB listener, and stores the public cert in Secret `scanner-tls-ca` + SSM `/<stack>/scanner-ca-cert`. Clients trust it via the SDK `ca_cert` arg (there is NO skip-verify option in the SDK — the cert MUST be supplied, and its SAN must match the connect host). Scanner-app supports this via `V1FS_TLS_ENABLED=true` + `V1FS_CA_CERT=/etc/v1fs-tls/ca.crt` (deployment mounts Secret `scanner-tls-ca` at `/etc/v1fs-tls`, `optional: true`). SDK `init()` real signature: `init(host, api_key, enable_tls=False, ca_cert=None)`
