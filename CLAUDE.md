...
> **THREE DEPLOYMENT OPTIONS (one template, `ScannerScalingMode` param).** `keda` (default) = KEDA scales the V1FS scanner on SQS **queue depth** (`k8s/scanner-scaledobject.yaml`, `v1fs-scanner-sqs-scaler`); chart HPA disabled; a customer variant, NOT Trend-supported — the latency-sensitive default (warm-floor `ScannerMinReplicas` + gentle ScaledObject scale-down). `hpa` = the chart's CPU/mem HPA (TrendAI-supported; select with `ScannerScalingMode=hpa`). `bootstrap.sh` resolves the mode into `SCANNER_KEDA` (keda needs a queue — full-auto's, or `ExternalScanQueueArn` in BYO; else falls back to hpa) and sets `scanner.autoscaling.enabled` accordingly; `deploy.sh` applies the scanner ScaledObject only when `SCANNER_KEDA=true`. **Upgrade discipline for keda:** always `scripts/upgrade.py` (never bare `helm upgrade`) — it re-asserts `autoscaling.enabled=false` and FAILS if a chart HPA reappears alongside KEDA (two autoscalers thrash). Client references live in `reference/{python-default,python-KEDA,java-KEDA}/`, each with its own POC-GUIDE.md; the KEDA ones include the pull/semaphore dispatcher (NLB-target-group discovery + per-pod pool). `ScannerEndpointMode=auto` → **NLB** (ALB opt-in); the NLB doubles as the pod-discovery registry. All options ARM-capable.
...
- **V1FS scanner autoscaling is `ScannerScalingMode`-driven** — `keda` (default): KEDA on queue depth (`k8s/scanner-scaledobject.yaml`, `v1fs-scanner-sqs-scaler`, threshold `SCANNER_QUEUE_LENGTH`), chart HPA disabled — a customer variant, NOT Trend-supported (the latency-sensitive default). `hpa`: chart CPU/mem HPA (`scanner.autoscaling.enabled=true` from `values-base.yaml` + min/max from `ScannerMinReplicas`/`ScannerMaxReplicas`), TrendAI-supported (select with `ScannerScalingMode=hpa`). `bootstrap.sh` computes `SCANNER_KEDA` (keda + a queue → true; keda-without-queue falls back to hpa) and sets `autoscaling.enabled` accordingly; `deploy.sh` applies the ScaledObject only when `SCANNER_KEDA=true`. **keda upgrade safety**: `upgrade.py` re-asserts `--set scanner.autoscaling.enabled=false` and its Step-6 guard **fails the upgrade** if a chart HPA reappears alongside the ScaledObject. NEVER bare `helm upgrade` in keda mode.
...
- **`scanDurationMs` splits into `acquireWaitMs` + `scanCallMs`** — the audit record's `scanDurationMs` is the total measured scan window and is UNCHANGED (existing consumers keep working). Two additive fields split it: `acquireWaitMs` = client-side wait for a free scanner-pod slot (`_acquire_least_busy` in pull mode; **0 in clusterip**), `scanCallMs` = the actual gRPC `scan_buffer` call. This lets you attribute a high `scanDurationMs` to fan-out queueing (`acquireWaitMs` dominates → scale scanner pods / lower `PER_POD_CAPACITY`) vs genuine scan cost (`scanCallMs` dominates). Both python and java flavors emit them; purely additive, no behavior change
