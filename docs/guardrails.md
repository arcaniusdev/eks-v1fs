...
- **Do not mark decompression-limit files clean.** They were NOT fully inspected. With the review pipeline they go to the review bucket; without it they go to QUARANTINE tagged `ScanResult=S3-DecompressionLimit` + `ScanErrors=<names>`. (Pre-realignment behavior copied them to the now-removed clean bucket when review was disabled — that was a bug. Clean files are now tagged in place in the ingest bucket; there is no clean bucket.)
...
- **Suspend bucket versioning on throwaway test/perf deployments.** The ingest and quarantine buckets are created with versioning enabled (Retain policy). A test feed overwrites objects and moves malicious files, so each object accrues versions plus delete-markers — a 10-minute load run leaves hundreds of thousands of versions. That makes teardown slow and error-prone: a plain `aws s3 rm --recursive` only removes current versions, so `rb` fails with `BucketNotEmpty`, and a partial version-purge can look finished when it is not. For test stacks, suspend versioning right after deploy — `aws s3api put-bucket-versioning --bucket <name> --versioning-configuration Status=Suspended` on the ingest and quarantine buckets — so overwrites don't accumulate versions. At teardown, always purge every version **and** delete-marker (paginated, batches of 1000) before `rb`, and re-list the bucket to confirm it is actually gone before declaring the account clean.
