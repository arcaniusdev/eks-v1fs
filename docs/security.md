...
- Review scanner IAM role (`ReviewScannerAppRole`) includes `s3:ListBucket` on the ingest bucket and `sqs:SendMessage` on the main FileScanQueue to support the reconciliation feature (detecting and re-queuing orphaned ingest files). It deliberately has no write permission to the review bucket — this enforces a one-way pipeline: review files are either tagged clean in place or moved to quarantine, never routed back to review. Even if `REVIEW_ROUTING_ENABLED` were misconfigured to `true`, the IAM policy would block the write and the file would fail to route, preventing infinite scanning loops
