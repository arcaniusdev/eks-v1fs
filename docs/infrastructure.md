...
- **`ScannerAppEnabled`** (`DeployScannerApp=true`): ECR repo, quarantine bucket, `FileScanQueue`/`FileScanDLQ` + queue policy, `ScannerAppRole` + Pod Identity, `ScanAuditLogGroup`, SNS alarm topic + DLQ/queue-age alarms, DLQ remediation Lambda, CloudWatch dashboard, KEDA Helm install + `KedaOperatorRole`
...
- **Single managed node group**: hosts BOTH system components and scanner workloads — one node tier, no per-workload nodeAffinity. `NodeInstanceType` default `r8g.xlarge` (Graviton ARM; allowed: r8g.xlarge, r7g.xlarge on ARM; r7i.xlarge, r7a.xlarge, r6i.xlarge, r7i.2xlarge on x86 — all memory-optimized, 4 vCPU / 32 GiB fits four 800m/2Gi V1FS scanner pods per node). The `NodeArchMap` template mapping sets the node group `AmiType` (AL2023 x86_64 or ARM64) and passes `TARGET_ARCH` to the bastion so `build-and-push.sh` builds the scanner-app image for the node architecture (QEMU binfmt cross-build when needed). Sizing: `NodeGroupMinSize`/`NodeGroupDesiredSize`/`NodeGroupMaxSize` = 2/2/8. The default max of 8 fits full-mode peak (10 V1FS scanners + 20 scanner-app pods + review pipeline + system pods)
...
- Clean files are NOT copied anywhere — they are left in place in the ingest bucket and tagged `ScanResult=S3-Clean`. There is no clean bucket.
